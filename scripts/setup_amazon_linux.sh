...
if [ ! -d "dpdk-23.11" ]; then
    ...
    cd dpdk-23.11
    # Disable GVE and IONIC drivers due to type conflict with system headers on AL2023
    meson setup build -Dexamples=helloworld "-Ddisable_drivers=net/gve,net/ionic"
    ninja -C build
